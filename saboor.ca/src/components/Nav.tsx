...
export const Nav = component$(() => {
  ...
  return <div class="fixed bottom-4 right-4 z-100" style={{
    ...
    <nav class={{
      ...
      <div class="lum-card lum-grad-bg-gray-900/50 backdrop-blur-lg p-1 gap-1 flex">
        <Socials class="rounded-lum-1" addLabels="right" color size={18} />
      </div>
      <div class="lum-card lum-grad-bg-gray-900/50 backdrop-blur-lg p-1 gap-1 flex">
        <Link href="/" class={{
          'lum-btn lum-bg-transparent rounded-lum-1': true,
          'lum-grad-bg-lum-accent hover:lum-bg-lum-accent': loc.url.pathname === '/',
        }}>
          <Laptop size={18} />
          Professional
        </Link>
        <Link href="/me" class={{
          'lum-btn lum-bg-transparent rounded-lum-1': true,
          'lum-grad-bg-lum-accent hover:lum-bg-lum-accent': loc.url.pathname.includes('/me'),
        }}>
          <Balloon size={18} />
          Personal
        </Link>
    ...
    <button class="lum-btn p-2 sm:p-4 lum-grad-bg-gray-900/50 backdrop-blur-lg mt-2" onClick$={() => (opened.value = !opened.value)}>
      <Menu size={32} />
    </button>
  </div>;
});
