    ...
    return (
      ...
      >
        ...
        <div class="rounded-lum-2 absolute inset-0 -z-10 h-full w-full overflow-clip object-cover saturate-200">
          <img
            class="absolute inset-0 -translate-y-1/3 animate-spin saturate-200 animation-duration-[10s]"
        ...
        >
          ...
          <div class="flex flex-1 flex-col text-xs">
            {activity.details && <p class="font-semibold">{activity.details}</p>}
            {activity.state && (
              <p class="overflow-hidden text-ellipsis text-gray-400">{activity.state}</p>
            )}
...
export const ExpandedCard = component$<ActivityCardProps>(({ activity, modalRef }) => {
  ...
  return (
    <div class="lum-card lum-grad-bg-gray-950/60 rounded-lum-2 relative h-full w-full p-12 transition-all duration-300">
      <div class="rounded-lum-2 absolute inset-0 -z-10 h-full w-full overflow-clip object-cover saturate-200">
        <img
          class="absolute top-0 right-0 -translate-y-1/3 scale-150 animate-spin saturate-200 animation-duration-[15s]"
          src={activity.assets?.large_image}
          alt={activity.assets?.large_text}
          width={1024}
          height={1024}
        />
        <img
          class="animate-spin-cc absolute bottom-0 left-0 translate-y-1/3 scale-150 saturate-200 animation-duration-[20s]"
          src={activity.assets?.large_image}
          alt={activity.assets?.large_text}
          width={1024}
          height={1024}
      ...
      <div class="mt-7 flex flex-1 flex-col text-xs">
        {activity.details && <p class="text-2xl font-semibold xl:text-4xl">{activity.details}</p>}
        {activity.state && (
        ...
        {activity.assets?.large_text && (
          <p class="text-lg text-gray-500 xl:text-xl">{activity.assets.large_text}</p>
        )}
        {activity.timestamps?.start && !activity.timestamps?.end && (
